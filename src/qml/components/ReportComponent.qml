...
//                    var lastData = showStartIndex > 0 ? tickData[showStartIndex - 1] : root.lastData;
//                    var lastChengJiaoLiang = lastData.ChengJiaoLiang || 0;
//                    var lastChengJiaoDanBiShu = lastData.ChengJiaoDanBiShu || 0;
                    var lastLabel, currentLabel, volume, tick, label, showId, updown, currentPrice, lastPrice;

                    return arr.map(function(eachData, index) {
                        volume = eachData.ChengJiaoLiang / stockCountUnit;
                        currentLabel = DateUtil.moment.unix(eachData.ShiJian).format('HH:mm:ss');
                        if (currentLabel === lastLabel) {
                            label = eachData.ZhubiId + 1;
                            showId = true;
                        } else {
                            label = currentLabel;
                            showId = false;
                        ...
                        currentPrice = eachData.ZuiXinJia;
                        if (lastPrice) {
                        ...
                        return {
                            id: [eachData.ShiJian, eachData.ZhubiId].join('_'),
                            time: eachData.ShiJian,
                            label: label,
                            showId: showId,
                            price: currentPrice,
                            volume: Math.abs(volume),
                            buySell: volume > 0 ? 1 : 0,
                            updown: updown,
            ...
            Rectangle {
                ...
                color: selected === modelData.id || (!selected && modelData.last) ? theme.rightSideBarHighLight : 'transparent'
                RowLayout {
                    spacing: 0
                    anchors.fill: parent
                    ...
                    Text {
                        ...
                        Layout.preferredWidth: tickLabelPreferredWidth + 20
                        color: tickLabelColor
                        font.pixelSize: modelData.showId ? tickLabelFontSize - 2 : tickLabelFontSize
