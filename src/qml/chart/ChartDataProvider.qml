...
Item {
    ...
    onCountChanged: {
        ...
        if (internal.cache.length < root.count) {
            if (root.hasMoreData) {
                var params = Util.assign({}, root.params);
                var requestCount = root.count - internal.cache.length;
    //            if (internal.cache.length > 0) {
    //                params.end_time = DateUtil.moment.unix(internal.cache[0].ShiJian).format('YYYYMMDD-HHmmss');
    //            } else {
                    params.start = -(internal.cache.length + requestCount);
    //            }
                ...
                for (i = data.length; i > 0; i--) {
                  ...
                  if (eachData.ShiJian < startTime) {
                    cache.unshift(eachData);
                      ...
                      console.log(root.serviceUrl, 'has same data')
